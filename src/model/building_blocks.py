...
class QueryGRUEncoder(nn.Module):
    def __init__(self, in_dim, dim, n_layers, dropout):
    ...
    def forward(self, query, query_mask, word_vectors):
        """
        Args:
            query: (B, L)
            query_mask: (B, L)
        ...
        words_feature = words_feature.contiguous()
        ...
        return words_feature, sentence_feature, glove_emb
...
class VideoSelfAttentionEncoder(nn.Module):
    ...
    def forward(self, video, video_mask):
        ...
        """
        temp, position_embeddings = self.video_embedding(video)
        video_embedding = temp
        for layer in self.layers:
            temp = layer(
                x=temp,
                mask=video_mask
            )
        return temp, video_embedding, position_embeddings
...
class QueryVideoCrossModalEncoderLayer(nn.Module):
    ...
    def __init__(self, dim, dropout):
        ...
        self.fc_q = PositionwiseFeedForward(dim=dim, d_ff=2*dim, dropout=dropout)
        self.fc_v = PositionwiseFeedForward(dim=dim, d_ff=2*dim, dropout=dropout)
        self.resize_para = nn.Parameter(torch.Tensor([1]))
        self.resize_para_2 = nn.Parameter(torch.Tensor([1]))

    def forward(self, query_feature, query_mask, video_feature, video_mask, theta=2.5):
        q2v, q2v_attn = self.q2v(
            query=theta*query_feature,
            key=theta*video_feature,
            value=video_feature,
