...
class Evaluator(object):
    ...
    def _res_to_string(self, res):
        return "\n".join([" | ".join([
                                         "R@{} IoU {}: {:.2f}".format(recall_at, thres, res[recall_at][thres] * 100) for
                                         thres in self.iou_threshold
                                     ] + ["mIoU: {:.2f}".format(res[recall_at]["miou"] * 100)]) for recall_at in
                          self.recall_at])
    ...
    def _res_to_score(res):
        ...
        """
        return sum([v for _, v in res[1].items()])

    def _update(self, preds, gts, video_id_array, epoch=0):
        """
        Args:
            preds: (N, topk, 2)
            gts: (N, 2)
        ...
        gts = gts.unsqueeze(1)
        intersection = torch.clamp(torch.min(preds[:, :, 1], gts[:, :, 1]) - torch.max(preds[:, :, 0], gts[:, :, 0]),
                                   min=0.0)
    ...
    def eval_dataloader(self, model, dataloader, epoch=0, total_epoch=1):
        model.eval_mode()
        preds = []
        gts = []
        video_id_array = []
        ...
        with torch.no_grad():
            for i, batch in enumerate(
                    tqdm(dataloader, total=len(dataloader), desc="Evaluating epoch {}".format(epoch))):
                B, _, _ = batch["video"].shape
                tot += B

                pred = model.forward_eval(batch)

                loss += model.forward_train_val(batch, epoch, total_epoch).item()
                preds.append(pred)
                gts.append(torch.stack([batch["start_frac"], batch["end_frac"]], dim=1))

                for ids in batch["video_id"]:
                    video_id_array.append(ids)

        loss /= len(dataloader)
        preds = torch.cat(preds, dim=0)
        gts = torch.cat(gts, dim=0)
        self._update(preds, gts, video_id_array, epoch)
...
def evaluate(exp_folder_path):
    ...
    evaluator = Evaluator()
    evaluator.eval_dataloader(model, test_dl, epoch=0, total_epoch=1)
